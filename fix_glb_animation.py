...
def import_glb(glb_path):
    ...
    print(f"[FIX] 임포트된 오브젝트들: {[obj.name for obj in objects]}")
    
    return objects

def find_armature():
    """Armature 찾기"""
    print("[FIX] Armature 검색 중...")
    
    armatures = [obj for obj in bpy.context.scene.objects if obj.type == 'ARMATURE']
    print(f"[FIX] Armature 오브젝트들: {[obj.name for obj in armatures]}")
    
    if armatures:
        target_armature = armatures[0]
        print(f"[FIX] 타겟 Armature 선택: {target_armature.name}")
    ...
    for mesh in meshes:
        if mesh.parent and mesh.parent.type == 'ARMATURE':
            print(f"[FIX] 메시의 부모 Armature 발견: {mesh.parent.name}")
            return mesh.parent
...
def import_bvh(bvh_path, target_armature):
    ...
    print(f"[FIX] 액션 할당 완료: {target_armature.name}")
    
    # 액션 정보 출력
    if action.fcurves:
        print(f"[FIX] 액션 프레임 범위: {action.frame_range}")
        print(f"[FIX] 액션 F-Curve 수: {len(action.fcurves)}")
        
        # 첫 번째 F-Curve 정보
        first_fcurve = action.fcurves[0]
        print(f"[FIX] 첫 번째 F-Curve: {first_fcurve.data_path} - {first_fcurve.array_index}")
        print(f"[FIX] 키프레임 수: {len(first_fcurve.keyframe_points)}")

def export_glb(output_path, start_frame=1, end_frame=100):
    """GLB 파일로 내보내기 (애니메이션 포함)"""
    print(f"[FIX] GLB 내보내기: {output_path}")
    print(f"[FIX] 프레임 범위: {start_frame} - {end_frame}")
    ...
    print(f"[FIX] GLB 내보내기 완료: {output_path}")
    
    # 파일 크기 확인
    file_size = Path(output_path).stat().st_size
    print(f"[FIX] 파일 크기: {file_size / 1024 / 1024:.2f} MB")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='GLB 애니메이션 수정 스크립트')
    ...
    print("[FIX] GLB 애니메이션 수정 스크립트 시작")
    print(f"[FIX] GLB: {args.glb}")
    print(f"[FIX] BVH: {args.bvh}")
    print(f"[FIX] 출력: {args.out}")
