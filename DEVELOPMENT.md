...
- 혈액형/질병 이력/알레르기 항목을 정리하고 수영 엔진과 동일한 10개 운동 목표 버튼을 적용해 가입 시 자동으로 목표가 연동되도록 갱신.
- 학생 계정이 아직 센터에 배정되지 않은 경우 `/api/centers/student-dashboard-stats`가 404를 반환해 가입 직후 대시보드가 실패하던 버그를 해결하고, 기본 0값과 함께 "센터 배정 대기" 안내 메시지를 노출하도록 `client/app/dashboard/page.tsx`를 개선.
...
## 🔍 자동 헬스 체크 (2025. 11. 13. 오후 11:21:34)
...
## 🔍 자동 헬스 체크 (2025. 11. 14. 오전 6:08:28)
...
### ✅ 2025-11-14: 센터 미배정 학생 자동 리다이렉트

- `client/app/dashboard/page.tsx`에서 학생이고 센터 미배정인 경우 대시보드 대신 `/map`(센터 검색) 페이지로 자동 리다이렉트하도록 수정.
- API 응답에서 `needsCenterAssignment: true` 또는 404 에러 메시지에 "소속 센터"가 포함된 경우, 학생 사용자는 바로 센터 검색 화면으로 이동.
- 가입 직후 또는 센터 미배정 상태에서는 사용자가 센터를 찾고 수강 신청할 수 있도록 UX 개선.

### ✅ 2025-11-14: 게스트 센터 목록 노출 버그 수정

- `/api/centers/guest`가 존재하지 않는 `isActive` 필드를 필터링하던 문제를 `status: 'active'` 조건으로 변경해 센터 데이터가 지도에 노출되도록 수정.
- `center@jjswimlab.com`이 관리하는 센터를 포함한 활성 센터들이 센터 검색 지도에 즉시 표시됨을 확인.
- 센터 검색 필터가 하드코딩된 샘플 목록만 대상으로 동작하던 문제를 주소·행정구역 정보 기반으로 개선해, `'전국'`이나 실제 행정구역을 선택해도 등록된 모든 센터가 정상적으로 남도록 조정.
- 센터 목록 애니메이션이 `animation`/`animationDelay` 속성을 동시에 갱신해 경고가 발생하던 현상을 개별 속성으로 분리해서 해결.
- 실시간 센터 데이터에 좌표가 빠져있는 경우에도 주소/이름 매칭 및 행정구역 키워드로 위치를 유추해, 강남구 테헤란로 등 실제 지점 근처로 지도 마커가 표시되도록 개선.
- 지도에서 선택한 센터에 대해 `센터 홈페이지`/`수강 신청` 버튼을 제공하고, 등록 링크가 없으면 이메일·전화로 바로 문의할 수 있도록 액션을 연결.
...
## 🔍 자동 헬스 체크 (2025. 11. 14. 오전 7:58:40)
...
## 🔍 자동 헬스 체크 (2025. 11. 14. 오전 8:06:02)
...
## 🔍 자동 헬스 체크 (2025. 11. 14. 오전 11:21:36)
...
## 🔍 자동 헬스 체크 (2025. 11. 14. 오후 12:10:33)
...
## 🔍 자동 헬스 체크 (2025. 11. 17. 오전 1:29:44)
