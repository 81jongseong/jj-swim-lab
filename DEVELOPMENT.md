...
### ✅ **센터 관리자 메뉴 재구성** (2025-11-02)
**진행 상태: 완료**

#### **작업 내용:**
1. **커뮤니티 카테고리 통합**
   - "지원 & 안내" 카테고리 제거
   - 이용안내, 공지사항, 구인구직을 커뮤니티 카테고리로 이동
   - 커뮤니티 카테고리에: 이용안내, 공지사항, 구인구직, 커뮤니티 포함

2. **추가 서비스 카테고리로 변경**
   - "도구" 카테고리명을 "추가 서비스"로 변경
   - 상점, 수영센터 찾기를 추가 서비스로 이동
   - 추가 서비스 카테고리에: 주간 프로그램 생성, 3D 뷰어, 상점, 수영센터 찾기 포함

#### **최종 센터 관리자 메뉴 구조:**
- **센터 관리**: 홈, 대시보드, 회원 관리, 강사 관리, 강의 관리, 통계, 공지사항 관리
- **예약·결제 관리**: 예약·결제 관리
- **센터 설정**: 센터 정보 관리, 사이트 테마 설정
- **커뮤니티**: 이용안내, 공지사항, 구인구직, 커뮤니티
- **추가 서비스**: 주간 프로그램 생성, 3D 뷰어, 상점, 수영센터 찾기

#### **수정된 파일:**
- `client/components/Navigation.tsx`: 센터 관리자 메뉴 구조 재구성

#### **커밋:**
- `feat: reorganize center admin menu - move items to community and rename tools category` (커밋 0ab83e5)
...
#### **커밋:**
- `fix: 자유수영 레인 대여 신청 문제 수정 및 평균수업시간 카드 삭제` (커밋 08aa890)
- `docs: 자유수영 레인 대여 신청 문제 수정 히스토리 추가` (커밋 8ff6726)

---

### ✅ **공휴일 운영시간 설정 기능 추가** (2025-10-31)
**진행 상태: 완료**

#### **작업 내용:**
- 센터 정보 관리 페이지에 공휴일 운영시간 설정 추가
- 평일/주말과 별도로 공휴일 운영시간 설정 가능
- 공휴일 운영 체크박스로 활성/비활성화 가능
- 공휴일은 기본 비활성화, 활성화 시에만 시간 설정 가능

#### **구현:**
1. **State 추가**
   - `holidaysOpen`, `holidaysClose`, `holidaysEnabled` state 추가

2. **UI 수정**
   - 운영시간 섹션을 3열 그리드로 변경 (평일/주말/공휴일)
   - 공휴일에 체크박스 추가로 운영 여부 설정
   - 체크 시에만 시작/종료 시간 입력 필드 표시

3. **데이터 로드 및 저장**
   - `loadCenterInfo`에서 `hours.holiday` 데이터 로드
   - `handleSave`에서 `operatingHours.holiday` 추가하여 저장
...
#### **구현:**
1. **건강 관리 메뉴 통합**
   - 센터 건강 현황, 건강 통계, 회원 건강정보 → "🏥 센터 건강 정보 통합 관리"로 통합
   - 건강 프로그램, 수영프로그램 관리 → 삭제 (센터 강의 관리로 대체)
   - 프로그램 이력 관리 → 삭제
   - 알고리즘 성과 → 삭제
   - 운동량 계산기 유지

2. **주간 프로그램 생성 메뉴 추가**
   - 최고관리자에만 있던 "수영 트레이닝 규칙 엔진" 메뉴를 센터 관리자에도 추가
   - "📅 주간 프로그램 생성" 이름으로 표시
   - `/admin/swim-training-engine` 경로 사용

#### **수정된 파일:**
- `client/components/Navigation.tsx`: 센터 관리자 메뉴 구조 개선
...
7. **건강 상태 추세 차트 구현**
   - BMI 표시 및 상태 표시 바 (저체중/정상/과체중/비만)
   - 체중/신장 추세 카드
   - 체중 변화 추세 그래프 (막대 그래프 형태)
   - 최근 측정일 표시
   - 인바디 차트 형태의 시각화
   - 두 개의 회원 관리 페이지에 모두 적용
   - `feat: add health status trend chart with BMI, weight, and height visualization` (커밋 adc1f7c)
