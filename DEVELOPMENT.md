...
**최종 해결 시도 (2025-11-02):**
1. ✅ `setTimeout` 제거 - 관리자 페이지처럼 바로 `setProps` 호출
2. ✅ `mapLoaded` 체크 추가 - 지도가 완전히 로드된 후에만 레이어 추가
3. ✅ 데이터 유효성 검사 강화 - 좌표 NaN 체크 추가
4. ✅ 위치 중복 확인 로그 추가 (관리자 페이지와 동일)
5. ✅ React StrictMode 이중 렌더링 방지 - `mapLoaded` 체크로 보호
