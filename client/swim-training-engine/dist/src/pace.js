 ...
 */
export function resolveBasePace(p) {
    if (p.cssSecPer100)
        return p.cssSecPer100;
    if (p.best100Sec)
        return p.best100Sec + 6; // 근사: best100 +6″ ≈ 임계
    if (p.z2SecPer100)
        return p.z2SecPer100 - 6; // 근사 변환
...
export function getPaceRange(zone, css) {
    const ranges = zonePace(css);
    return ranges[zone];
}
export function calculateZoneDistribution(totalMeters, zones) {
    // 목표별 존 분배 비율
    const zoneRatios = {
        Z1: 0.20, // 20% 회복
        Z2: 0.50, // 50% 기초 지구력
...
export function formatPaceNote(pace, zone) {
    const formattedPace = formatPace(pace);
    return `@ ${formattedPace} (${zone})`;
}
export function calculateRestTime(intensity, distance) {
    // 거리와 강도에 따른 휴식 시간 계산
    const baseRest = {
        Z1: 10, // 회복: 짧은 휴식
        Z2: 15, // 기초: 중간 휴식
...
export function adjustPaceByRPE(basePace, rpe) {
    // RPE에 따른 페이스 조정
    // RPE 6-7: 기본 페이스
    // RPE 8-9: 5-10초 빠르게
    // RPE 4-5: 5-10초 느리게
