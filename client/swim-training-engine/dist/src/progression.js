 ...
 * 다음 주 계획 조정
 */
export function nextProgression(currentPlan, progressionData, userInput) {
 ...
 * 사용자 입력 조정
 */
function adjustUserInput(userInput, adjustmentFactor) {
    const adjustedInput = { ...userInput };
    // 세션 시간 조정
    adjustedInput.avail.sessionMinutes = Math.round(userInput.avail.sessionMinutes * adjustmentFactor);
    // 페이스 조정 (RPE 기반)
    if (userInput.pace.cssSecPer100) {
        adjustedInput.pace.cssSecPer100 = adjustPaceByRPE(userInput.pace.cssSecPer100, 6, // 목표 RPE
        7 // 실제 RPE (예시)
        );
    }
 ...
 * 주간 진행률 계산
 */
export function calculateWeeklyProgress(plannedSessions, completedSessions) {
    const totalPlannedMeters = plannedSessions.reduce((sum, session) => sum + (session.reps * session.distance), 0);
    const totalCompletedMeters = completedSessions.reduce((sum, session) => sum + (session.reps * session.distance), 0);
 ...
 * 평균 RPE 계산
 */
function calculateAverageRPE(completedSessions) {
 ...
 * 진행률 기반 노트 생성
 */
function generateProgressNotes(completionRate, averageRPE) {
    const notes = [];
    if (completionRate >= 0.9) {
 ...
 * 개인별 맞춤 조정
 */
export function personalizePlan(basePlan, userProfile) {
    ...
    if (userProfile.health.hypertension) {
        // 고혈압: 고강도 세트 제한
        personalizedPlan.sessions.forEach(session => {
            session.sets = session.sets.filter(set => !set.paceNote.includes('Z4') && !set.paceNote.includes('Z5'));
        });
    }
    if (userProfile.health.obesity) {
        // 비만: 킥 세트 제한
        personalizedPlan.sessions.forEach(session => {
            session.sets = session.sets.filter(set => !set.drillIds?.includes('kick_board') &&
 ...
 * 목표별 조정
 */
export function adjustForGoal(plan, goal) {
 ...
 * 계절별 조정
 */
export function adjustForSeason(plan, season) {
    const adjustedPlan = { ...plan };
    switch (season) {
        case 'spring':
            // 봄: 점진적 증가
