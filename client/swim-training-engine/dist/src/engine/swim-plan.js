const { medicalClearanceNeeded, weeklyDoseMinutes, levelSessionRange, rpePrimary, hrSecondary, BP_STOP_RULE } = require('./health-policy');
const { allJointConditions } = require('../data/jj-swim-lab-joint-guidance');
...
function buildPlan(i) {
    ...
    for (let d = 0; d < days; d++) {
        ...
        const blocks = [
            ...
            { stroke: (main[1] ?? 'freestyle'), block: `${Math.max(6, Math.round(perSession * 0.5))}' @${rpePrimary()}` },
