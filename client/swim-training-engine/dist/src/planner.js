 ...
 * - 페이스/레스트/드릴/코칭큐 포함
 */
import { resolveBasePace, zonePace, formatPaceNote, calculateRestTime, calculateZoneDistribution } from './pace';
...
function buildSession(params) {
    const { dayIndex, totalMeters, basePace, zonePaces, zoneDist, safetyCaps, avail, goal, stroke, technique, health } = params;
    const sets = [];
 ...
 * 워밍업 세트 생성
 */
function createWarmupSet(meters, stroke, technique) {
 ...
 * 메인 세트 생성
 */
function createMainSets(meters, zoneDist, zonePaces, basePace, stroke, goal, safetyCaps) {
    const sets = [];
    // 목표별 메인 세트 구성
    switch (goal) {
        case 'fatloss':
 ...
 * 체중감량 목표 세트 생성
 */
function createFatlossSets(meters, zonePaces, basePace, stroke, safetyCaps) {
    const sets = [];
    // Z2 지속 세트 (60%)
 ...
 * 지구력 목표 세트 생성
 */
function createEnduranceSets(meters, zonePaces, basePace, stroke, safetyCaps) {
    const sets = [];
    // Z2 지속 세트 (70%)
 ...
 * 기록향상 목표 세트 생성
 */
function createPerformanceSets(meters, zonePaces, basePace, stroke, safetyCaps) {
    const sets = [];
    // Z3 템포 세트 (40%)
 ...
 * 존별 세트 생성
 */
function createZoneSet(zone, meters, zonePaces, basePace, stroke, description) {
 ...
 * 쿨다운 세트 생성
 */
function createCooldownSet(meters, stroke) {
    const reps = Math.ceil(meters / 50);
    const distance = 50;
    return {
        label: 'Cool-down',
 ...
 * 총 거리 계산
 */
function calculateTotalMeters(avail, goal) {
 ...
 * 기술 드릴 세트 생성
 */
function createTechniqueSet(meters, stroke, technique) {
 ...
 * 하이폭식 세트 생성 (안전 범위 내)
 */
function createHypoxicSet(meters, stroke, safetyCaps) {
