...
// ========================== Query/Help API ==========================
export function listDrillsByStroke(stroke) { return DRILLS.filter(d => d.strokes.includes(stroke)); }
export function listDrillsBySkill(target) { return DRILLS.filter(d => d.skillTargets.includes(target)); }
export function listTrainingTypes() { return TRAININGS; }
export function getHelp(kind, id) {
    if (kind === 'drill') {
        const d = DRILLS.find(x => x.id === id);
        if (!d)
            return '드릴을 찾을 수 없습니다.';
...
// ========================== Extensibility ==========================
export function addDrill(newDrill) { DRILLS.push(newDrill); }
export function addTrainingType(newType) { TRAININGS.push(newType); }
//# sourceMappingURL=drill-library.js.map