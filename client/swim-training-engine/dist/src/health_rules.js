 ...
 */
// 관절 질환별 안전 규칙
export const JOINT_SAFETY_RULES = {
    // 무릎 질환
    'knee_osteoarthritis': {
        condition: '무릎 골관절염',
        restriction: 'caution',
...
// 건강 플래그별 안전 규칙
export const HEALTH_SAFETY_RULES = {
    hypertension: {
        condition: '고혈압',
        restriction: 'caution',
 ...
 * 건강 플래그 기반 안전 제한 적용
 */
export function getSafetyCaps(health) {
    let maxIntensity = 100; // 기본값
    const restrictedMethods = [];
    const restrictedDrills = [];
    const modifications = [];
    // 고혈압 체크
 ...
 * 관절 질환별 안전 규칙 조회
 */
export function getJointSafetyRule(condition) {
    return JOINT_SAFETY_RULES[condition] || null;
}
/**
 * 건강 플래그별 안전 규칙 조회
 */
export function getHealthSafetyRule(condition) {
    return HEALTH_SAFETY_RULES[condition] || null;
}
/**
 * 의학적 근거 조회
 */
export function getMedicalEvidence(level) {
    if (level) {
        return MEDICAL_EVIDENCE.filter(evidence => evidence.level === level);
    }
    return MEDICAL_EVIDENCE;
}
//# sourceMappingURL=health_rules.js.map