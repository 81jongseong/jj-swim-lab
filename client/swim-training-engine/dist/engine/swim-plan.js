...
const health_policy_1 = require("./health-policy");
const jj_swim_lab_joint_guidance_1 = require("../data/jj-swim-lab-joint-guidance");
function pickSafeStrokes(orthos) {
    ...
    for (const cid of orthos) {
        const cond = jj_swim_lab_joint_guidance_1.allJointConditions.find(c => c.conditionId === cid);
        if (!cond)
            continue;
        for (const s of strokes) {
...
function buildPlan(i) {
    const clearance = (0, health_policy_1.medicalClearanceNeeded)(i);
    const weeklyMin = (0, health_policy_1.weeklyDoseMinutes)(i);
    const [minPer, maxPer] = (0, health_policy_1.levelSessionRange)(i.swim_profile.level);
    ...
    for (let d = 0; d < days; d++) {
        const main = strokes.slice(0, 2).length ? strokes.slice(0, 2) : ['backstroke', 'elementary_backstroke'];
        const blocks = [
            { stroke: (main[0] ?? 'backstroke'), block: `${Math.round(perSession * 0.3)}' easy` },
            { stroke: (main[1] ?? 'freestyle'), block: `${Math.max(6, Math.round(perSession * 0.5))}' @${(0, health_policy_1.rpePrimary)()}` },
            { stroke: 'elementary_backstroke', block: `${Math.round(perSession * 0.2)}' easy` }
        ];
        sessions.push({
            day: daysOfWeek[d],
            focus: i.goals,
            ...
            intensity_cues: { primary: (0, health_policy_1.rpePrimary)(), secondary: (0, health_policy_1.hrSecondary)(i) },
            stop_rules: [health_policy_1.BP_STOP_RULE, 'chest_pain', 'unusual_dyspnea']
        });
    }
    let next;
    ...
    const finalSessions = clearance ? [{
            day: 'Mon',
            focus: ['safety_hold'],
            stroke_plan: [{ stroke: 'elementary_backstroke', block: '10\' very easy' }, { stroke: 'backstroke', block: '10\' easy' }],
