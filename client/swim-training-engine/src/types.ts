...
export interface HealthInput {
  demographics:{ age:number; sex:Sex; };
  anthropometrics:{ height_cm:number; weight_kg:number; waist_cm?:number; bmi?:number; };
  vitals:{ rest_hr?:number; rest_bp?:{sbp:number;dbp:number}; meds?:string[]; on_beta_blocker?:boolean; };
  labs?:{ tc?:number; ldl?:number; hdl?:number; tg?:number; fpg?:number; hba1c?:number; egfr?:number; };
  conditions:{
    obesity:'normal'|'overweight'|'obesity';
    hypertension:'normal'|'elevated'|'stage1'|'stage2';
    dyslipidemia:boolean; diabetes:boolean;
  };
  orthopedics:string[];
  swim_profile:{ level:SwimLevel; swim_hr_peak_land?:number|null; rpe_tolerance?:string; };
  goals:string[];
  adherence_last_week:number;
  symptoms_flags:string[];
}

export interface StrokeBlock { stroke: Stroke; block: string; }
export interface SessionPlan {
  day: string;
  focus: string[];
  stroke_plan: StrokeBlock[];
  constraints: string[];
  intensity_cues: { primary:string; secondary?:string; };
  stop_rules: string[];
}
