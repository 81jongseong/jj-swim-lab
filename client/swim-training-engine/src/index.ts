 ...
 * 엔진 초기화
 */
export function initializeEngine(): Promise<boolean> {
 ...
 * 계획 생성 (새로운 API)
 */
export function generateSwimPlan(input: UserInput): WeekPlan {
  try {
    // 입력 검증
 ...
 * 계획 요약 생성
 */
export function generatePlanSummary(plan: WeekPlan): {
  totalMeters: number;
  ...
  const zoneDistribution = Object.entries(plan.summary.zoneDist).reduce(
    (acc, [zone, meters]) => {
