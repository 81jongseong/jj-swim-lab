...
import { calculateExercisePrescription, generateWorkoutBlocks, getPaceDescription, getIntensityDescription } from './exercise-calculator';
...
function handleSpecialConditions(healthData: any): { strokes: Stroke[], constraints: string[], intensityReduction: number } {
  ...
  if (specialConditions?.pregnancy?.isPregnant) {
    ...
    const conditionData = specialConditionsData.find(
      (data: any) => data.conditionId === `pregnancy_${trimester}_trimester`
    );
  ...
  if (specialConditions?.postSurgery?.hasSurgery) {
    ...
    const conditionData = specialConditionsData.find(
      (data: any) => data.conditionId === `post_${surgeryType}_surgery_${recoveryStage}`
    );
...
function generateStrokePlan(
  strokes: Stroke[], 
  sessionDuration: number, 
  swimLevel: string,
  grade: string = '3급',
  intensityReduction: number = 0,
  poolDistance: number = 25
): {stroke: Stroke; block: string; distance?: number; duration?: number; pace?: number}[] {
  // 운동량 계산 시스템 사용
  const prescription = calculateExercisePrescription(
    swimLevel as any,
    sessionDuration,
    strokes,
    intensityReduction,
...
export function buildPlan(i:HealthInput): PlanOutput {
  ...
  const constraints:string[] = [...specialConditionsResult.constraints];
  ...
  const exercisePrescription = calculateExercisePrescription(
    i.swim_profile.level,
    adjustedPerSession, // 세션당 시간 사용
    strokes,
    specialConditionsResult.intensityReduction,
    i.swim_profile.grade || '3급',
    i.swim_profile.poolDistance || 25
  );

  for (let d=0; d<days; d++) {
    const strokePlan = generateStrokePlan(
      strokes, 
      adjustedPerSession, // 세션당 시간 사용
      i.swim_profile.level,
      i.swim_profile.grade || '3급',
  ...
  const finalSessions = clearance? [{
    ...
    focus:['safety_hold'],
    stroke_plan:[
      {stroke:'elementary_backstroke' as Stroke, block:'10분 매우 가벼운 워밍업', distance: 100, duration: 10, pace: 180},
      {stroke:'backstroke' as Stroke, block:'10분 가벼운 운동', distance: 100, duration: 10, pace: 150}
    ],
