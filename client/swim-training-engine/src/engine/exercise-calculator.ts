...
export interface ExercisePrescription {
  ...
  pace: number;                 // 페이스 (초/100m)
  intensity: number;            // 운동 강도 (%)
  restRatio: number;           // 휴식 비율 (%)
  strokeDistribution: {        // 영법별 분배
    [key: string]: {
      distance: number;
      duration: number;
      pace: number;
...
export const PACE_STANDARDS: SwimPaceStandards = {
  beginner: {
...
export function calculateExercisePrescription(
  swimLevel: SwimLevel,
  ...
  // 급수별 조정 계수 적용
  const gradeAdjustment = GRADE_ADJUSTMENT[grade as keyof typeof GRADE_ADJUSTMENT] || GRADE_ADJUSTMENT['3급'];
  ...
  // 영법별 분배 계산
  const strokeDistribution: { [key: string]: { distance: number; duration: number; pace: number } } = {};
  ...
  availableStrokes.forEach(stroke => {
    const pace = paceStandards[stroke as keyof typeof paceStandards] * gradeAdjustment.pace;
...
export function generateWorkoutBlocks(
  prescription: ExercisePrescription,
  availableStrokes: string[],
  poolDistance: number = 25
): Array<{ stroke: string; block: string; distance: number; duration: number }> {
  const blocks: Array<{ stroke: string; block: string; distance: number; duration: number }> = [];
...
export function getPaceDescription(pace: number): string {
  if (pace <= 80) return '매우 빠름 (경쟁 수준)';
  if (pace <= 100) return '빠름 (고급자)';
  if (pace <= 120) return '보통 (중급자)';
  if (pace <= 140) return '느림 (초급자)';
  return '매우 느림 (입문자)';
}

export function getIntensityDescription(intensity: number): string {
  if (intensity >= 80) return '고강도';
  if (intensity >= 60) return '중강도';
  if (intensity >= 40) return '저강도';
  return '매우 저강도';
}
