/**
 * 🏊‍♂️ JJ Swim Lab - 수영 프로그램 생성기 타입 정의
 * 
 * 📋 **핵심 기능:**
 * - 건강·질환·기술 기반 개인별 수영 프로그램 생성
 * - 랩 단위(25m/50m) 세트, 페이스/레스트/드릴/코칭큐 포함
 * - 안전 게이트 및 진행(Progression) 로직
 * 
 * 🔗 **연동 파일:**
 * - pace.ts (페이스 변환 규칙)
 * - training_methods.ts (훈련법 카탈로그)
 * - drills.ts (드릴 카탈로그)
 * - health_rules.ts (건강 규칙)
 * - planner.ts (세션 생성 알고리즘)
 */
