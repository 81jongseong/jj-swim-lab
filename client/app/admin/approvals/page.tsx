...
export default function ApprovalsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">✅ 승인 대기 관리</h1>
        <p className="text-gray-600 mt-2">JJ Swim Lab 승인 요청 현황 및 처리</p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>📋 승인 요청 목록 ({filteredApprovals.length}건)</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <ResponsiveTable>
              ...
              <TableBody>
                {filteredApprovals.map((approval) => (
                  <div key={approval.id} className="grid grid-cols-7 gap-4 p-4 border-b border-gray-200">
                    <div>{getTypeLabel(approval.type)}</div>
                    <div>
                      <div className="font-medium">{approval.title}</div>
                      <div className="text-sm text-gray-500">{approval.description}</div>
                      {approval.estimatedAmount && (
                        <div className="text-sm text-green-600 font-medium">
                          {approval.estimatedAmount.toLocaleString()}원
                        </div>
                      )}
                    </div>
                    <div>
