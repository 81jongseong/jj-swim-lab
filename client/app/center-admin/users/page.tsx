...
import MemberManagementModal from '@/components/center-admin/MemberManagementModal';
import MemberBulkImportModal from '@/components/center-admin/MemberBulkImportModal';
...
function CenterUsersManagement() {
  ...
  const handleUpdateMemo = async (memberId: string, memo: string, memoType: string) => {
    try {
      ...
      if (response.ok) {
        // 메모 저장 후 회원 목록 다시 로드하여 새로고침 없이 메모 이력 업데이트
        loadMembers();
      }
    } catch (error) {
  ...
  const handleCourseAssignment = async (memberId: string, courseId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`과정 배정에 실패했습니다: ${errorData.message || '알 수 없는 오류'}`);
      }
    } catch (error) {
  ...
  const handleSaveMember = async (memberId: string, updatedData: Partial<Member>) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/center-admin/members/${memberId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
