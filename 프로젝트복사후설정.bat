...
call npm run type-check:client >nul 2>&1
if errorlevel 1 (
...
call npm run type-check:server >nul 2>&1
if errorlevel 1 (
...
echo 🔍 ESLint 검사...
call npm run lint:client >nul 2>&1
call npm run lint:server >nul 2>&1
...
echo 📦 번들 크기 분석...
cd client
call npm run bundle:analyze >nul 2>&1
cd ..

echo 📊 성능 체크...
cd client
call npm run performance >nul 2>&1
cd ..
...
echo --------------------------------
echo 🔌 서버를 시작합니다...
start "JJ Swim Lab Server" cmd /k "cd server && npm run dev"

timeout /t 3 /nobreak >nul

echo 📱 클라이언트를 시작합니다...
start "JJ Swim Lab Client" cmd /k "cd client && npm run dev"
...
echo    2. MongoDB Atlas 연결 확인
echo    3. CI/CD 파이프라인 테스트
echo.
echo 💡 PowerShell 버전도 나중에 제공 예정입니다!
echo.
pause
