...
try:
    # 사용자 모델 로드
    user_model_path = r"C:\Users\user\jj-swim-lab\server\uploads\processed\video-1757070403419-867242908\user_model.blend"
    user_model = None
    ...
    if file_ext == '.fbx':
        ...
        if armatures:
            ...
            # 2. 사용자 업로드 모델만 선택 (Blender 기본 오브젝트 제외)
            user_objects = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']
            ...
            user_uploaded_objects = [obj for obj in user_objects if obj.name not in default_objects]
            print("DEBUG 사용자 업로드 모델들: " + str([obj.name for obj in user_uploaded_objects]))
            
            # 사용자가 업로드한 모델만 선택 (Blender 기본 오브젝트 제외)
            if user_uploaded_objects:
                # 각 오브젝트의 버텍스 수 확인
                for obj in user_uploaded_objects:
                    vertex_count = len(obj.data.vertices) if obj.data else 0
                    print("DEBUG 사용자 모델 " + obj.name + " 버텍스 수: " + str(vertex_count))
                
                # 가장 큰 메시 선택 (버텍스 수 기준)
                user_model = max(user_uploaded_objects, key=lambda obj: len(obj.data.vertices) if obj.data else 0)
                print("SUCCESS 사용자 모델 선택 (사용자 업로드): " + user_model.name)
            else:
                # 사용자 업로드 모델이 없으면 기본 모델 생성
                print("WARNING 사용자 업로드 모델이 없습니다. 기본 모델을 생성합니다.")
                create_default_model()
    ...
    bvh_path = r"C:\Users\user\jj-swim-lab\server\uploads\processed\video-1757070403419-867242908\motion.bvh"
...
# 모델 내보내기
glb_path, fbx_path, preview_path = export_model(r"C:\Users\user\jj-swim-lab\server\uploads\processed\video-1757070403419-867242908", "video-1757070403419-867242908")
