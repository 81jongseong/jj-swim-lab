/**
 * 🏊 SwimLab - 프로그램 완료율 API
 * 
 * 📋 **라우트 목적**
 * - 세션별 완료율 입력/수정/조회
 * - 프로그램 완료율 이력 및 분석
 * - 권한 기반 완료율 관리 (본인/강사)
 * 
 * 🔄 **연동되는 데이터**
 * - SwimProgram 모델 (세션별 completion 필드)
 * - User 모델 (권한 확인)
 * 
 * 💡 **주요 엔드포인트**
 * - GET /api/swim-programs/:programId/sessions - 세션 목록 조회
 * - POST /api/swim-programs/:programId/sessions/:sessionIndex/completion - 완료율 입력
 * - PUT /api/swim-programs/:programId/sessions/:sessionIndex/completion - 완료율 수정
 * - GET /api/swim-programs/:programId/completion-history - 완료율 이력
 */

import express from 'express';
import mongoose from 'mongoose';
import SwimProgram from '../models/SwimProgram';
import { User } from '../models/User';
import { authMiddleware } from '../middleware/auth';

const router = express.Router();

/**
 * 권한 확인 헬퍼 함수
 */
async function canEditCompletion(
  currentUserId: string,
  targetUserId: string,
  program: any
): Promise<boolean> {
  // 1. 본인의 프로그램
  if (currentUserId === targetUserId) {
    return true;
  }
  
  // 2. 프로그램을 생성한 강사
  if (program.createdBy && program.createdBy.toString() === currentUserId) {
    return true;
  }
  
  // 3. 담당 강사 확인 (PT 회원)
  const member = await User.findById(targetUserId);
  if (member && (member as any).assignedInstructor && (member as any).assignedInstructor.toString() === currentUserId) {
    return true;
  }
  
  // 4. 그룹 강사 확인 (단체반 회원)
  if (member && (member as any).assignedGroups && Array.isArray((member as any).assignedGroups)) {
    for (const group of (member as any).assignedGroups) {
      if (group.instructor && group.instructor.toString() === currentUserId) {
        return true;
      }
    }
  }
  
  // 5. 현재 로그인한 사용자가 강사인지 확인 (추가 권한)
  const currentUser = await User.findById(currentUserId);
  if (currentUser && ((currentUser as any).userType === 'instructor' || (currentUser as any).userType === 'centerAdmin')) {
    return true;
  }
  
  return false;
}

/**
 * GET /api/swim-programs/:programId/sessions
 * 프로그램의 모든 세션 조회 (완료율 포함)
 */
router.get('/:programId/sessions', authMiddleware, async (req, res, next) => {
  try {
    const { programId } = req.params;
    const currentUserId = (req as any).user?._id;

    const program = await SwimProgram.findById(programId);
    if (!program) {
      return res.status(404).json({
        success: false,
        message: '프로그램을 찾을 수 없습니다'
      });
    }

    // 권한 확인
    const canEdit = await canEditCompletion(
      currentUserId,
      program.athleteId ? program.athleteId.toString() : currentUserId,
      program
    );

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const sessions = program.content.sessions.map((session: any, index: number) => {
      const sessionDate = session.date ? new Date(session.date) : null;
      let status: 'upcoming' | 'available' | 'completed' = 'upcoming';
      
      if (sessionDate) {
        if (sessionDate <= today) {
          status = session.completion ? 'completed' : 'available';
        }
      }

      return {
        sessionIndex: index,
        sessionId: `${programId}_session_${index}`,
        day: session.day,
        date: session.date,
        themeDesc: session.themeDesc,
        status,
        completion: session.completion ? {
          completionRate: session.completion.completionRate,
          feeling: session.completion.feeling,
          inputBy: session.completion.inputBy,
          inputByRole: session.completion.inputByRole,
          inputAt: session.completion.inputAt,
          notes: session.completion.notes
        } : undefined,
        canEdit: canEdit && status === 'available'
      };
    });

    res.json({
      success: true,
      data: {
        programId,
        athleteId: program.athleteId,
        athleteName: program.athleteName,
        sessions
      }
    });
  } catch (error) {
    console.error('세션 조회 오류:', error);
    next(error);
  }
});

/**
 * POST /api/swim-programs/:programId/sessions/:sessionIndex/completion
 * 세션 완료율 입력
 */
router.post('/:programId/sessions/:sessionIndex/completion', authMiddleware, async (req, res, next) => {
  try {
    const { programId, sessionIndex } = req.params;
    const currentUserId = (req as any).user?._id;
    const {
      completionType,
      simpleCompletion,
      detailedCompletion,
      inputByRole
    } = req.body;

    const program = await SwimProgram.findById(programId);
    if (!program) {
      return res.status(404).json({
        success: false,
        message: '프로그램을 찾을 수 없습니다'
      });
    }

    // 권한 확인
    const canEdit = await canEditCompletion(
      currentUserId,
      program.athleteId ? program.athleteId.toString() : currentUserId,
      program
    );

    if (!canEdit) {
      return res.status(403).json({
        success: false,
        message: '완료율을 입력할 권한이 없습니다'
      });
    }

    const sessionIdx = parseInt(sessionIndex);
    if (sessionIdx < 0 || sessionIdx >= program.content.sessions.length) {
      return res.status(400).json({
        success: false,
        message: '잘못된 세션 인덱스입니다'
      });
    }

    let completionRate = 0;
    let feeling: 'easy' | 'moderate' | 'hard' | 'very_hard' = 'moderate';
    let notes = '';
    let detailedSets = undefined;

    // 간편 입력
    if (completionType === 'simple' && simpleCompletion) {
      completionRate = simpleCompletion.overallRate;
      feeling = simpleCompletion.feeling || 'moderate';
      notes = simpleCompletion.notes || '';
    }
    // 상세 입력
    else if (completionType === 'detailed' && detailedCompletion) {
      // 세트별 완료율 계산 (총 거리 기반)
      const sets = detailedCompletion.sets || [];
      if (sets.length > 0) {
        const totalPlannedDistance = sets.reduce((sum: number, set: any) => {
          return sum + (set.planned.distance * set.planned.reps);
        }, 0);
        const totalActualDistance = sets.reduce((sum: number, set: any) => {
          return sum + (set.actual.distance * set.actual.reps);
        }, 0);
        completionRate = totalPlannedDistance > 0 
          ? Math.min(Math.round((totalActualDistance / totalPlannedDistance) * 100), 100)
          : 0;
        
        console.log('📊 백엔드 완료율 계산:', {
          totalPlannedDistance,
          totalActualDistance,
          completionRate,
          sets: sets.map((s: any) => ({
            planned: s.planned.distance * s.planned.reps,
            actual: s.actual.distance * s.actual.reps
          }))
        });
      }
      feeling = detailedCompletion.feeling || 'moderate';
      notes = detailedCompletion.notes || '';
      detailedSets = sets;
    }

    // 완료율 저장
    program.content.sessions[sessionIdx].completion = {
      completionRate,
      feeling,
      inputBy: new mongoose.Types.ObjectId(currentUserId),
      inputByRole: inputByRole || 'self',
      inputAt: new Date(),
      notes,
      detailedSets
    };

    await program.save();

    res.json({
      success: true,
      message: '완료율이 입력되었습니다',
      data: {
        sessionId: `${programId}_session_${sessionIdx}`,
        completion: {
          completionRate,
          calculatedFrom: completionType
        }
      }
    });
  } catch (error) {
    console.error('완료율 입력 오류:', error);
    next(error);
  }
});

/**
 * GET /api/swim-programs/:programId/completion-history
 * 프로그램 완료율 이력 및 분석
 */
router.get('/:programId/completion-history', authMiddleware, async (req, res, next) => {
  try {
    const { programId } = req.params;

    const program = await SwimProgram.findById(programId);
    if (!program) {
      return res.status(404).json({
        success: false,
        message: '프로그램을 찾을 수 없습니다'
      });
    }

    // 완료된 세션만 필터링
    const completedSessions = program.content.sessions.filter((s: any) => s.completion);
    
    const sessions = completedSessions.map((session: any) => ({
      date: session.date,
      day: session.day,
      completionRate: session.completion.completionRate,
      feeling: session.completion.feeling,
      inputBy: session.completion.inputBy,
      inputByRole: session.completion.inputByRole,
      inputAt: session.completion.inputAt,
      notes: session.completion.notes
    }));

    // 평균 완료율 계산
    const avgCompletionRate = completedSessions.length > 0
      ? Math.round(completedSessions.reduce((sum: number, s: any) => 
          sum + s.completion.completionRate, 0) / completedSessions.length)
      : 0;

    // 추세 계산 (최근 3개 세션)
    let trend: 'improving' | 'stable' | 'declining' = 'stable';
    if (completedSessions.length >= 3) {
      const recent = completedSessions.slice(-3);
      const rates = recent.map((s: any) => s.completion.completionRate);
      const isImproving = rates[2] > rates[0] && rates[1] >= rates[0];
      const isDeclining = rates[2] < rates[0] && rates[1] <= rates[2];
      
      if (isImproving) trend = 'improving';
      else if (isDeclining) trend = 'declining';
    }

    // 간단한 추천
    let nextIntensity = 1.0;
    let nextVolume = 1.0;
    let reason = '';

    if (avgCompletionRate >= 90) {
      nextIntensity = 1.03;
      nextVolume = 1.03;
      reason = '완료율이 우수합니다. 강도와 볼륨을 3% 증가시킬 수 있습니다.';
    } else if (avgCompletionRate >= 80) {
      nextIntensity = 1.0;
      nextVolume = 1.0;
      reason = '완료율이 양호합니다. 현재 강도를 유지하세요.';
    } else if (avgCompletionRate >= 70) {
      nextIntensity = 0.97;
      nextVolume = 0.95;
      reason = '완료율이 다소 낮습니다. 강도를 3%, 볼륨을 5% 감소시키는 것을 권장합니다.';
    } else {
      nextIntensity = 0.90;
      nextVolume = 0.85;
      reason = '완료율이 낮습니다. 강도와 볼륨을 크게 줄여 기초를 다시 다지세요.';
    }

    res.json({
      success: true,
      data: {
        programId,
        athleteId: program.athleteId,
        completedSessions: completedSessions.length,
        totalSessions: program.content.sessions.length,
        avgCompletionRate,
        trend,
        sessions,
        recommendation: {
          nextIntensity,
          nextVolume,
          reason,
          scientificBasis: [
            'TRIMP (Training Impulse) 모델',
            'Banister Fitness-Fatigue 모델',
            '점진적 과부하 원칙'
          ]
        }
      }
    });
  } catch (error) {
    console.error('완료율 이력 조회 오류:', error);
    next(error);
  }
});

/**
 * GET /api/swim-programs/athlete/:athleteId/incomplete-sessions
 * 완료율 미입력 세션 조회
 */
router.get('/athlete/:athleteId/incomplete-sessions', authMiddleware, async (req, res, next) => {
  try {
    const { athleteId } = req.params;
    const currentUserId = (req as any).user?._id;

    const programs = await SwimProgram.find({ 
      athleteId: new mongoose.Types.ObjectId(athleteId)
    }).sort({ createdAt: -1 }).limit(10);

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const incompleteSessions: any[] = [];

    for (const program of programs) {
      const canEdit = await canEditCompletion(
        currentUserId,
        athleteId,
        program
      );

      program.content.sessions.forEach((session: any, index: number) => {
        const sessionDate = session.date ? new Date(session.date) : null;
        
        if (sessionDate && sessionDate <= today && !session.completion) {
          const daysAgo = Math.floor((today.getTime() - sessionDate.getTime()) / (1000 * 60 * 60 * 24));
          
          incompleteSessions.push({
            programId: program._id,
            sessionIndex: index,
            athleteName: program.athleteName,
            session: {
              sessionId: `${program._id}_session_${index}`,
              day: session.day,
              date: session.date,
              daysAgo
            },
            canEdit
          });
        }
      });
    }

    res.json({
      success: true,
      data: {
        incompleteSessions
      }
    });
  } catch (error) {
    console.error('미입력 세션 조회 오류:', error);
    next(error);
  }
});

export default router;

