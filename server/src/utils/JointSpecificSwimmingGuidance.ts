/**
 * 🏊‍♂️ JJ Swim Lab - 관절별 수영 가이드라인 시스템
 * 
 * 📋 **파일 개요**
 * - 실제 의료진이 필요로 하는 구체적인 관절별 수영 가이드라인
 * - 각 관절의 질환별 권장/금지 수영법
 * - 의학적 근거 기반 안전 수영 가이드
 * 
 * 🔗 **연동 데이터**
 ...
 * 
 * 📅 **개발 히스토리**
 * - 2025-01-22: 관절별 구체적 수영 가이드라인 시스템 구현
 */
...
// 관절별 수영 가이드라인 클래스
export class JointSpecificSwimmingGuidance {
  
  /**
   * 척추(허리) 관련 질환별 수영 가이드라인
   */
  ...
  /**
   * 특정 관절과 질환에 대한 수영 가이드라인 조회
   */
  static getSwimmingGuidanceForCondition(
    joint: string, 
    condition: string
  ): {
    condition: JointCondition;
    swimmingGuidance: { [stroke: string]: SwimmingSafety };
    generalRecommendations: string[];
    contraindications: string[];
    rehabilitationTips: string[];
  } | null {
  ...
  static getRecommendedStrokes(
    ...
    if (!guidance) {
      return ['freestyle', 'backstroke']; // 기본 추천
    }
