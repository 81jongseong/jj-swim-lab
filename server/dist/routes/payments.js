...
router.get('/pricing/calculate', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('요금 계산 오류', error);
...
router.get('/:id', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('결제 조회 오류', error);
        return res.status(500).json({ error: '서버 오류가 발생했습니다.' });
    }
});
router.post('/', auth_1.auth, async (req, res) => {
    try {
        ...
        try {
            ...
            (0, logger_1.logError)('결제 처리 중 오류', error);
        }
        ...
        (0, logger_1.logError)('결제 생성 오류', error);
        return res.status(500).json({ error: '서버 오류가 발생했습니다.' });
    }
});
...
router.post('/:id/refund', auth_1.auth, (0, auth_1.requireRole)(['superAdmin']), async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('결제 환불 오류', error);
        return res.status(500).json({ error: '서버 오류가 발생했습니다.' });
    }
});
router.get('/stats/summary', auth_1.auth, (0, auth_1.requireRole)(['superAdmin']), async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('결제 통계 조회 오류', error);
        return res.status(500).json({ error: '서버 오류가 발생했습니다.' });
    }
});
router.get('/course/:courseId/stats', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('강습 과정별 결제 통계 조회 오류', error);
...
router.get('/student/:studentId/courses', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('학생별 강습 과정 결제 내역 조회 오류', error);
...
router.get('/instructor/:instructorId/courses', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('강사별 강습 과정 결제 통계 조회 오류', error);
        res.status(500).json({ error: '결제 통계 조회에 실패했습니다.' });
    }
});
router.get('/course/:courseId/stats', auth_1.auth, async (req, res) => {
...
router.get('/pricing/policy', auth_1.auth, (0, auth_1.requireRole)(['superAdmin']), async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('요금 정책 조회 오류', error);
...
router.put('/pricing/policy', auth_1.auth, (0, auth_1.requireRole)(['superAdmin']), async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('요금 정책 업데이트 오류', error);
...
router.get('/pricing/discount/:userId', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, logger_1.logError)('할인율 조회 오류', error);
