...
router.get('/personalized-recommendations', auth_1.auth, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user._id);
        if (!user) {
            return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
...
router.get('/performance-insights', auth_1.auth, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user._id);
        if (!user) {
            return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
...
router.get('/community', auth_1.auth, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user._id);
        if (!user) {
            return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
...
async function getStudentDashboard(user) {
    try {
        ...
        const completedCourses = await Course_1.Course.find({
        ...
        const checklistCompletionRate = totalChecklistItems > 0
            ...
            : 0;
        const evaluations = await Evaluation_1.Evaluation.find({
            student: user._id
        })
            .populate('course', 'name')
            .populate('instructor', 'name')
            .sort({ createdAt: -1 })
            .limit(3);
        const centerInfo = await SwimmingCenter_1.SwimmingCenter.findOne({
            students: user._id
...
async function getCenterAdminDashboard(user) {
    try {
        const centerAdmin = await User_1.User.findById(user._id);
        ...
        const centerInfo = await SwimmingCenter_1.SwimmingCenter.findById(centerId)
            .select('name address currentCapacity maxCapacity');
        const recentNotices = await Notice_1.Notice.find({ center: centerId })
            .sort({ createdAt: -1 })
            .limit(5);
        return {
            user: {
...
async function getSuperAdminDashboard(user) {
    try {
        const [totalUsers, totalCenters, totalCourses, totalBookings] = await Promise.all([
            User_1.User.countDocuments(),
            SwimmingCenter_1.SwimmingCenter.countDocuments(),
            Course_1.Course.countDocuments(),
            Booking_1.Booking.countDocuments()
        ]);
        const userTypeStats = await User_1.User.aggregate([
            {
                $group: {
                    _id: '$userType',
                    count: { $sum: 1 }
