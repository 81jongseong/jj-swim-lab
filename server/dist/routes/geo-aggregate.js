...
router.get('/aggregate', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const skipNoise = noNoise === 'true' || (typeof noNoise === 'boolean' && noNoise);
        const skipRound = noRound === 'true' || (typeof noRound === 'boolean' && noRound);
        ...
        if (user.userType === 'centerAdmin' || user.userType === 'center-admin') {
            ...
            if (centerId !== 'all' && centerId !== null && centerId !== undefined && centerId !== '') {
                ...
                if (mongoose_1.default.Types.ObjectId.isValid(centerIdStr)) {
                    filter.centerId = centerIdStr;
                }
                else {
                    (0, logger_1.logInfo)('센터 이름으로 ObjectId 조회 시도', { centerId, centerIdType: typeof centerId });
                    const center = await Center_1.default.findOne({ name: centerId }).select('_id name').lean();
                    if (center) {
                        filter.centerId = center._id;
                        (0, logger_1.logInfo)('센터 이름으로 ObjectId 변환 성공', {
                            centerName: centerId,
                        ...
                        (0, logger_1.logWarn)('센터를 찾을 수 없음', { centerId, centerIdType: typeof centerId });
        ...
        for (const userItem of users) {
            ...
            if (!coords && userItem.centerId) {
                try {
                    ...
                    let centerIdObj = centerId;
                    if (typeof centerId === 'string' && mongoose_1.default.Types.ObjectId.isValid(centerId)) {
                        centerIdObj = new mongoose_1.default.Types.ObjectId(centerId);
                    }
                    const { SwimmingCenter } = await Promise.resolve().then(() => __importStar(require('../models/SwimmingCenter')));
                    center = await SwimmingCenter.findById(centerIdObj).select('address location').lean();
                    if (!center) {
                        center = await Center_1.default.findById(centerIdObj).select('address location').lean();
                    }
                    if (processedCount < 3) {
        ...
        cells.forEach(cell => {
            ...
            if (skipNoise && skipRound) {
                ...
                cell.countApprox = addNoiseAndRound(originalCount, 1.0);
            }
            delete cell.count;
