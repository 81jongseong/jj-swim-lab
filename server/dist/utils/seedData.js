...
exports.runSeedData = exports.seedTestData = void 0;
...
const seedTestData = async () => {
    try {
        ...
        let center = await Center_1.default.findOne({ name: 'JJ Swim Lab 메인 센터' });
        if (!center) {
        ...
        let adminUser = await User_1.User.findOne({ email: 'admin@jjswimlab.com' });
        if (!adminUser) {
            adminUser = new User_1.User({
                name: '시스템 관리자',
                email: 'admin@jjswimlab.com',
                password: '$2a$12$2y/oGMSJuxuQsLAbAcgrse.lIKP4bj58zoKe7wGZNJP44F6mF02M2',
                userType: 'admin',
                centerId: center._id,
                isActive: true,
                profile: {
        ...
        let centerAdminUser = await User_1.User.findOne({ email: 'center@jjswimlab.com' });
        if (!centerAdminUser) {
            centerAdminUser = new User_1.User({
                name: '센터 관리자',
        ...
        for (const instructorData of instructors) {
            let instructor = await User_1.User.findOne({ email: instructorData.email });
            if (!instructor) {
                instructor = new User_1.User({
        ...
        for (const studentData of students) {
            let student = await User_1.User.findOne({ email: studentData.email });
            if (!student) {
        ...
        for (const productData of products) {
            let product = await Product_1.default.findOne({ name: productData.name });
            if (!product) {
        ...
        for (const recData of exerciseRecommendations) {
            let recommendation = await ExerciseRecommendation_1.default.findOne({
                technique: recData.technique,
                level: recData.level,
                category: recData.category
            });
            if (!recommendation) {
                recommendation = new ExerciseRecommendation_1.default({
                    ...recData,
                    centerId: center._id,
                    createdBy: centerAdminUser._id
                });
        ...
        const orders = [
            {
                customerId: (await User_1.User.findOne({ email: 'student1@example.com' }))?._id,
                customerName: '최학생',
                customerEmail: 'student1@example.com',
                customerPhone: '010-7777-8888',
                items: [
                    {
                        productId: (await Product_1.default.findOne({ name: '수영복 (남성용)' }))?._id,
                        productName: '수영복 (남성용)',
                        quantity: 1,
                        price: 45000
                    },
                    {
                        productId: (await Product_1.default.findOne({ name: '수영모자' }))?._id,
                        productName: '수영모자',
                        quantity: 2,
                        price: 15000
                    }
        ...
        for (const orderData of orders) {
            if (orderData.customerId) {
                let order = await Order_1.default.findOne({
...
const runSeedData = async () => {
    try {
        const success = await (0, exports.seedTestData)();
        if (success) {
